...
export class CatalogRepositoryService {
  ...
  enroll(classId: string): Observable<any> {
    if (!this.userRepositoryService.currentUser)
      return throwError(() => new Error('User not signed in'));

    if (this.userRepositoryService.currentUser.classes.includes(classId))
      return throwError(() => new Error('Already enrolled'));

    this.userRepositoryService.currentUser.classes.push(classId);
  ...
  drop(classId: string): Observable<any> {
    ...
    this.userRepositoryService.currentUser.classes = this.userRepositoryService.currentUser.classes.filter((c: string) => c !== classId);
